function f = objective_PG3_LE_1(z,points,points2,points3,vmax,vdes,plagerror,...
    platerror, pprog, pab, pdotbeta, pspeedcost,pspeedcostMax,pslack,pslack2)
    global index
    ...
    speedcost = max(z(index.v)-vdes,0)^2*pspeedcost;
    speedcost1 = min(z(index.v)-vdes,0)^2*pprog;
    lagcost = plagerror*lagerror^2;
    latcost = pslack*latErrorPunisher(laterror);
    speedcost2 = max(z(index.v)-vmax,0)^2*pspeedcostMax;
    latcost1 = platerror*laterror1^2;
    %prog = -pprog*z(index.ds);
    regAB = z(index.dotab).^2*pab;
    regBeta= z(index.dotbeta).^2*pdotbeta;
        
    speedcost_k2 = max(z(index.v_k2)-vdes,0)^2*pspeedcost;
    speedcost1_k2 = min(z(index.v_k2)-vdes,0)^2*pprog;
    speedcost2_k2 = max(z(index.v_k2)-vmax,0)^2*pspeedcostMax;
    ...
    speedcost_k3 = max(z(index.v_k3)-vdes,0)^2*pspeedcost;
    speedcost1_k3 = min(z(index.v_k3)-vdes,0)^2*pprog;
    speedcost2_k3 = max(z(index.v_k3)-vmax,0)^2*pspeedcostMax;
    ...
    %prog_k3 = -pprog*z(index.ds_k3);
     ...
     f = (lagcost   +latcost   +latcost1   +regAB   +regBeta   +speedcost1   +speedcost   +speedcost2)+...prog+pslack*slack+
         ...
         pslack2*slack2+pslack2*slack3+pslack2*slack4;%+prog_k3+pslack*slack_k3+
end
