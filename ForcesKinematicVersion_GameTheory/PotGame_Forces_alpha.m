...
model2 = model;
% for i=1:model2.N
%    model2.objective{i} = @(z,p)objective_simplePG_alpha1(z,p(index.pslack),...
%                         p(index.xend1),p(index.xend2),p(index.yend1),...
%                         p(index.yend2),p(index.alpha));
% end
...
for i =1:tend
    for kk=1:length(alpha)
        ...
        problem.all_parameters = repmat(getParameters_simplePGalpha(pslack,...
            dist,xend1,yend1,xend2,yend2,alpha(kk)), model.N ,1); 
        ...
        for jj=1:length(outputM)
                ...
                [u1cost_v1,u2cost_v1,u1cost_v2,u2cost_v2,xcost1,xcost2,ycost1,...
                 ycost2,slackcost,f,f1,f2] = objective_PG_simpleTestAlpha(...
                                    out,pslack,xend1,xend2,yend1,yend2,alpha(kk));
        ...
        figure(1)
        plot(outputM(:,index.x_v1),outputM(:,index.y_v1),'.-','Color',[0,alpha(kk),1])
        plot(outputM(:,index.x_v2),outputM(:,index.y_v2),'.-','Color',[1,alpha(kk),0])
        
        figure(2)
        plot(outputM(:,index.u_acc_v1),'.-','Color',[0,alpha(kk),1])
        plot(outputM(:,index.u_acc_v2),'.-','Color',[1,alpha(kk),0])
        
        figure(3)
        plot(outputM(:,index.u_steer_v1),'.-','Color',[0,alpha(kk),1])
        plot(outputM(:,index.u_steer_v2),'.-','Color',[1,alpha(kk),0])
...
figure(4)
for kk=1:length(alpha)
plot(alpha(kk),optA(kk),'*','Color',[0,alpha(kk),1])
plot(alpha(kk),optB(kk),'*','Color',[1,alpha(kk),0])
end
% %%
% 
%  
...
% legend ('acc','steer')
% dist=(outputM(:,index.x_v1)-outputM2(:,index.x_v2)).^2+...
%      (outputM(:,index.y_v1)-outputM2(:,index.y_v2)).^2;