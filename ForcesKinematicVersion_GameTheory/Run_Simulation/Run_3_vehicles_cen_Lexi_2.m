...
for i =1:tend
    ...
    problem.x0 = x0(:);       
    % solve mpc
    [output,exitflag,info] = MPCPathFollowing_3v_N1(problem);
    ...
    problem1.x0 = x0(:);       
    % solve mpc
    [output1,exitflag1,info1] = MPCPathFollowing_3v_N2(problem1);
    solvetimes1(end+1)=info1.solvetime;
    if(exitflag1==0)
       a1 = a1 + 1; 
    end
...
%         plot(outputM(:,index.x_k3),outputM(:,index.y_k3),'Color',[0,1,0],'Linewidth',3)
        ...
        for ii=1:length(outputM1(1:P_H_length,index1.x))-1
            if abs(outputM1(ii,index1.ab))<1e-2
                outputM1(ii,index1.ab)=0;
            end
            if abs(outputM1(ii,index1.ab_k2))<1e-2
                outputM1(ii,index1.ab_k2)=0;
            end
            if abs(outputM1(ii,index1.ab_k3))<1e-2
                outputM1(ii,index1.ab_k3)=0;
            end
...
%             vc2 = outputM2(iff,index_IBR.ab)/maxxacc2;
%             vc3 = outputM3(iff,index_IBR.ab)/maxxacc3;
            theta = atan2(outputM1(iff+1,index1.y)-outputM1(iff,index.y),outputM1(iff+1,index1.x)-outputM1(iff,index1.x)); % to rotate 90 counterclockwise
            R = [cos(theta) -sin(theta); sin(theta) cos(theta)];
        ...
        axis equal
        savefig('figures/3v_PG_intersection1')
        saveas(gcf,'figures/3v_PG_intersection1','epsc')
        int=integrator_stepsize;
%         figure(2)
%         plot(int:int:length(outputM(:,1))*int,outputM(:,index.theta),'b.-','Linewidth',1)
%         plot(int:int:length(outputM(:,1))*int,outputM(:,index.theta_k2),'r.-','Linewidth',1)
...
%         legend('Kart1','Kart2','Kart3')
        ...
        %legend('Vehicle 1','V 2','V 3')
        set(gca,'FontSize',18)
        savefig('figures/3v_PG_speed1')
        saveas(gcf,'figures/3v_PG_speed1','epsc')
        % drawAnimation_P3_PH
%         figure(1)
%         hold on
