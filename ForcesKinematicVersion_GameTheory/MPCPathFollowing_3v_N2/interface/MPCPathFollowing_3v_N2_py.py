...
a Python wrapper for a fast solver generated by FORCES PRO v4.1.1
   ...
   OUTPUT = MPCPathFollowing_3v_N2_py.MPCPathFollowing_3v_N2_solve(PARAMS) solves a multistage problem
   subject to the parameters supplied in the following dictionary:
       PARAMS['x0'] - column vector of length 1980
       PARAMS['xinit'] - column vector of length 21
       PARAMS['all_parameters'] - column vector of length 8520

   OUTPUT returns the values of the last iteration of the solver where
       OUTPUT['alldata'] - column vector of size 1980

   [OUTPUT, EXITFLAG] = MPCPathFollowing_3v_N2_py.MPCPathFollowing_3v_N2_solve(PARAMS) returns additionally
   ...
   [OUTPUT, EXITFLAG, INFO] = MPCPathFollowing_3v_N2_py.MPCPathFollowing_3v_N2_solve(PARAMS) returns 
...
#_lib = ctypes.CDLL(os.path.join(os.getcwd(),'MPCPathFollowing_3v_N2/lib/MPCPathFollowing_3v_N2.dll')) 
try:
    _lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'MPCPathFollowing_3v_N2/lib/MPCPathFollowing_3v_N2_withModel.dll'))
    csolver = getattr(_lib,'MPCPathFollowing_3v_N2_solve')
except:
    _lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'MPCPathFollowing_3v_N2/lib/libMPCPathFollowing_3v_N2_withModel.dll'))
    csolver = getattr(_lib,'MPCPathFollowing_3v_N2_solve')

class MPCPathFollowing_3v_N2_params_ctypes(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
    _fields_ = [('x0', ctypes.c_double * 1980),
('xinit', ctypes.c_double * 21),
('all_parameters', ctypes.c_double * 8520),
]

MPCPathFollowing_3v_N2_params = {'x0' : np.array([]),
'xinit' : np.array([]),
...
MPCPathFollowing_3v_N2_params_types = {'x0' : np.float64,
'xinit' : np.float64,
'all_parameters' : np.float64,
}

class MPCPathFollowing_3v_N2_outputs_ctypes(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
    _fields_ = [('alldata', ctypes.c_double * 1980),
]

MPCPathFollowing_3v_N2_outputs = {'alldata' : np.array([]),
}


class MPCPathFollowing_3v_N2_info(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
...
csolver.argtypes = ( ctypes.POINTER(MPCPathFollowing_3v_N2_params_ctypes), ctypes.POINTER(MPCPathFollowing_3v_N2_outputs_ctypes), ctypes.POINTER(MPCPathFollowing_3v_N2_info), ctypes.POINTER(FILE))
csolver.restype = ctypes.c_int

def MPCPathFollowing_3v_N2_solve(params_arg):
    '''
a Python wrapper for a fast solver generated by FORCES PRO v4.1.1
 ...
 See also COPYING
    ...
    # convert parameters
    params_py = MPCPathFollowing_3v_N2_params_ctypes()
    for par in params_arg:
        try:
            ...
            if isinstance(getattr(params_py, par), ctypes.Array):
                params_arg[par] = np.require(params_arg[par], dtype=MPCPathFollowing_3v_N2_params_types[par], requirements='F')
    ...
    outputs_py = MPCPathFollowing_3v_N2_outputs_ctypes()
    info_py = MPCPathFollowing_3v_N2_info()
    if sys.version_info.major == 2:
        ...
        try:
            PyFile_AsFile.restype = ctypes.POINTER(FILE)
            exitflag = _lib.MPCPathFollowing_3v_N2_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), PyFile_AsFile(fp) , _lib.MPCPathFollowing_3v_N2_casadi2forces )
        ...
        try:
            if sys.platform.startswith('win'):
                exitflag = _lib.MPCPathFollowing_3v_N2_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), None , _lib.MPCPathFollowing_3v_N2_casadi2forces)
            else:
                exitflag = _lib.MPCPathFollowing_3v_N2_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), fp , _lib.MPCPathFollowing_3v_N2_casadi2forces)
            libc.fclose(fp)
    ...
    # convert outputs
    for out in MPCPathFollowing_3v_N2_outputs:
        MPCPathFollowing_3v_N2_outputs[out] = npct.as_array(getattr(outputs_py,out))

    return MPCPathFollowing_3v_N2_outputs,int(exitflag),info_py

solve = MPCPathFollowing_3v_N2_solve
